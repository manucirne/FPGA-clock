...
class Line_Assemble:
    ...
    def get_instruction(self):
        ...
        if self.get_instruction_type() == 'r':
            output = self.get_r_instruction(instruct) + self.get_register(
                args[2]) + self.get_register(args[1]) + self.get_register(args[0])
        elif self.get_instruction_type() == 'i' and len(args) == 2:

            if instruct == "lea":
                output = self.get_i_instruction(instruct) + self.get_register(
                    args[1]) + self.get_register(args[0]) + self.get_immediate(args[0], 8)
            else:
                output = self.get_i_instruction(instruct) + self.get_register(
                    args[0]) + self.get_register(args[1]) + self.get_immediate(args[1], 8)
        elif self.get_instruction_type() == 'i' and len(args) == 3:
            output = self.get_i_instruction(instruct) + self.get_register(
                args[2]) + self.get_register(args[1]) + self.get_immediate(args[0], 8)
        elif self.get_instruction_type() == 'j' and len(args) == 1:
            output = self.get_j_instruction(
                instruct) + "0"*8 + self.get_j_address(args[0])
        elif self.get_instruction_type() == 'j' and len(args) == 2:
            output = self.get_j_instruction(
                instruct) + self.get_register(args[0]) + self.get_j_address(args[1])
    ...
    def get_register(self, register):
        ...
        table = {'zero': '0', 't0': '1', 't1': '2', 't2': '3', 't3': '4', 't4': '5', 't5': '6', 't6': '7',
                 ...
                 'cr0': '17', 'cr1': '18', 'cr2': '19', 'cr3': '1a', 'cr4': '1b', 'cr5': '1c', 'am': '1d', 'mod': '1e', 'pm': '1f',
                 'rl6': '20', 'rl7': '21', 'cr': '22'
                 }
...
class MIPS_MIF_Format:
    ...
    def end(self):
        if self.current_addr < 2**self.addr-1:
            self.stream.write('[{}..{}]:   {};\n'.format(
                self.current_addr, 2**self.addr-1, ''.zfill(self.data_width)))
...
class MIPS_Assemble:
    ...
    def assemble(self):
        ...
        for _, l in enumerate(self.read_stream):
...
if __name__ == '__main__':
    ...
    argparse.add_argument('-m', '--mif-format',
    ...
    if args.mif_format:
        out_format = MIPS_MIF_Format(
            addr=args.addr, data_width=args.width, increment_by=args.increment_by)
